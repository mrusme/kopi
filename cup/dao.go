...
func (dao *DAO) Create(
	...
	id, err := dal.Create(ctx, dao.dal.DB(),
		"INSERT INTO "+Table()+" ("+Columns(false)+") VALUES (?, ?, ?);",
		&entity.CoffeeID,
		&entity.Drink,
		&entity.Timestamp,
	)
...
func (dao *DAO) GetByID(
	...
	return dal.GetRow[Cup](ctx, dao.dal.DB(),
		"SELECT "+Columns(true)+" FROM "+Table()+" WHERE `id` = ? LIMIT 1;",
		id,
	)
}
...
func (dao *DAO) FindByIDs(
	...
	q := fmt.Sprintf(
		"SELECT "+Columns(true)+" FROM "+Table()+" WHERE `id` IN (%s) ORDER BY `id`;",
		placeholders,
	)
...
func (dao *DAO) FindIDsWithDrink(
	...
	return dal.FindColumns[int64](ctx, dao.dal.DB(),
		"SELECT `id` FROM "+Table()+" WHERE `drink` = ?;",
		drink,
	)
}
